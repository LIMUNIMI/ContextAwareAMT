...
class AEBatchSampler(Sampler):
    ...
    def __next__(self):
        ...
        batch = np.argwhere(
            np.logical_and(
                self.enc_dataset.not_used, (self.enc_dataset.sample_contexts == c)
            )
        )
...
def process_pedaling(i, dataset, nmf_params):
    ...
    hop = s.HOP_SIZE / s.SR
    pedaling = (
        dataset.get_pedaling(i, frame_based=True, winlen=winlen, hop=hop)[0] / 127
    )
...
def process_velocities(i, dataset, nmf_params):
    ...
    nmf_tools.to2d()
    velocities = (
        dataset_utils.get_score_mat(dataset, i, score_type=["precise_alignment"])[:, 3]
        / 127
    )  # type: ignore
